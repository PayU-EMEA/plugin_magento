...
*   CHANGE_LOG:
*   	2012-05-09:
*   		- When self-return not enabled changed force status change from cancel to reject
*   		- added rejectOrder method
*		2012-03-22:
*			- One Step logic moved to model   
*       2012-03-16:
*       	- added order validity time verification and setting
*       2012-03-16:
*       	- added PayU order status update (cancelPayment and completePayment)
*       2012-03-16:
*       	- initialization of PayU configuration moved to constructor
*   	2012-03-12:
*   		- resetting the billing information only to guest customers
*   	2012-03-05: 
*			- update shipping and billing for guest users
*       2012-03-01:
*       	- order statuses management rewritten
*       	- shipment change after PayU notification
*       2012-02-28:
*           - orderTaxRate calculation rewritten
*           - recalculating shipment costs rewritten
*/

require_once('lib/payu/sdk/openpayu.php');

class PayU_PayU_Model_Payment extends Mage_Payment_Model_Method_Abstract

{
	/**
	 *
	 * Configuration
	 * @var PayU_PayU_Model_Config
	 */
	protected $_config;
	...
	const NEW_PAYMENT_URL = "payu/payment/new";
	...
	public function orderCreateRequest(Mage_Sales_Model_Order $order, $allShippingRates){
		...
		if ( $result->getSuccess() ) {
			...
			Mage::throwException( $this->__('There was a problem with initializing the payment, please contact the store administrator.' . $result->getError() ));
		}
	...
	public function newOneStep(){
		...
		->setCustomerIsGuest(true)->setCustomerEmail(md5(rand().rand())."_TEMP_PayU@".Mage::getModel('payu/config')->getDomainName())
		...
		$checkout->savePayment(array('method'=>'payu'));
	...
	public function beforeSummary(){
		...
		if ( $result->getSuccess() ) {
			...
			Mage::throwException( Mage::helper('payu')->__('There was a problem with the payment initialization, please contact system administrator.').$result->getError() );
		}
	...
	public function completeOrder($order){
		$this->_order = $order;
		$result = $this->orderStatusUpdateRequest(PayU_PayU_Model_Payment::ORDER_STATUS_COMPLETE);
		if($result){
			$this->updateOrderStatus(PayU_PayU_Model_Payment::ORDER_STATUS_COMPLETE);
			return $result;
		}
	}
	...
	public function rejectOrder($order){
		$this->_order = $order;
		$result = $this->orderStatusUpdateRequest(PayU_PayU_Model_Payment::ORDER_STATUS_REJECT);
		if($result){
			$this->updateOrderStatus(PayU_PayU_Model_Payment::ORDER_STATUS_REJECT);
			return $result;
		}
	}
	...
	public function cancelOrder($order){
		$this->_order = $order;
		$result = $this->orderStatusUpdateRequest(PayU_PayU_Model_Payment::ORDER_STATUS_CANCEL);
		if($result){
			$this->updateOrderStatus(PayU_PayU_Model_Payment::ORDER_STATUS_CANCEL);
			return $result;
		}
	}
	...
	protected function updateOrderStatus($orderStatus){
			...
			try{
				switch($orderStatus)
				{
					...
					case PayU_PayU_Model_Payment::ORDER_STATUS_CANCEL :
						$this->updatePaymentStatusCancelled($payment);
						break;

					case PayU_PayU_Model_Payment::ORDER_STATUS_REJECT :
						$this->updatePaymentStatusDenied($payment);
						break;

					case PayU_PayU_Model_Payment::ORDER_STATUS_SENT :
						$this->updatePaymentStatusInProgress($payment);
						break;

					case PayU_PayU_Model_Payment::ORDER_STATUS_PENDING :
		...
		if($currentState != $paymentStatus)
		{
			try{
				switch($paymentStatus)
				{
					...
					case PayU_PayU_Model_Payment::PAYMENT_STATUS_NEW :
						$this->updatePaymentStatusNew($payment);
						break;
							
					case PayU_PayU_Model_Payment::PAYMENT_STATUS_CANCEL :
						$this->updatePaymentStatusCancelled($payment);
						break;

					case PayU_PayU_Model_Payment::PAYMENT_STATUS_REJECT :
						$this->updatePaymentStatusDenied($payment);
						break;

					case PayU_PayU_Model_Payment::PAYMENT_STATUS_REJECT_DONE :
						$this->updatePaymentStatusReturned($payment);
						break;

					case PayU_PayU_Model_Payment::PAYMENT_STATUS_END :
						$this->updatePaymentStatusCompleted($payment);
						break;

					case PayU_PayU_Model_Payment::PAYMENT_STATUS_ERROR :
	 ...
	 *
	 * @return PayU_PayU_Model_Session
	 */
	public function getSession()
	{
		return Mage::getSingleton('payu/session');
	}
	...
	protected function toAmount($val){
		return Mage::helper('payu')->toAmount($val);
	}
	...
	public function updatePaymentStatusNew($payment)
	{
		...
		$transaction->setPreparedMessage( "PayU - " . Mage::helper('payu')->__('New transaction started.') );
		$transaction->save();
	}
	...
	public function updatePaymentStatusCancelled($payment)
	{
		...
		$payment->setPreparedMessage( "PayU - " . Mage::helper('payu')->__('Transaction cancelled.') )
		->registerVoidNotification();
			
		$comment = $this->_order->setState(Mage_Sales_Model_Order::STATE_HOLDED, true, "PayU - " . Mage::helper('payu')->__('The transaction has been cancelled.'), true)
	...
	public function updatePaymentStatusDenied($payment)
	{
		...
		$payment->setPreparedMessage( "PayU - " . Mage::helper('payu')->__('Transaction rejected.') )
		->setParentTransactionId($this->_transactionId)
		->registerVoidNotification();
			
		$comment = $this->_order->setState(Mage_Sales_Model_Order::STATE_HOLDED, true, "PayU - " . Mage::helper('payu')->__('The transaction has been rejected.'), true)
		->sendOrderUpdateEmail()
		->save();
	...
	public function updatePaymentStatusInProgress($payment)
	{
		...
		$payment->setPreparedMessage( "PayU - " . Mage::helper('payu')->__('The transaction is in progress.') )
	...
	public function updatePaymentStatusPending($payment)
	{
		...
		$payment->setPreparedMessage( "PayU - " . Mage::helper('payu')->__('Transaction awaits approval.')  )
		->setParentTransactionId($this->_transactionId) // this is the authorization transaction ID
		->registerVoidNotification();
			
		$comment = $this->_order->setState(Mage_Sales_Model_Order::STATE_PENDING_PAYMENT, true, "PayU - " . Mage::helper('payu')->__('The transaction is pending.') , false)
		->save();
	...
	public function updatePaymentStatusReturned($payment)
	{
		...
		$payment->setPreparedMessage( "PayU - " . Mage::helper('payu')->__('Transaction returned.')  )
		->registerVoidNotification();

		$comment = $this->_order->setState(Mage_Sales_Model_Order::STATE_HOLDED, true, "PayU - " . Mage::helper('payu')->__('The transaction has been returned.') , true)
		->sendOrderUpdateEmail()
		->save();
	}
	...
	public function updatePaymentStatusCompleted($payment)
	{
		...
		$transaction = $payment->setTransactionId($this->_transactionId);
		$transaction->setPreparedMessage( "PayU - " . Mage::helper('payu')->__('The transaction completed successfully.') );

		$payment->setIsTransactionApproved(true);
		$payment->setIsTransactionClosed(true);
			
		$comment = $this->_order->setState(Mage_Sales_Model_Order::STATE_PROCESSING, true, "PayU - " . Mage::helper('payu')->__('The transaction completed successfully.'), true)
		->sendOrderUpdateEmail(true, "PayU - " . Mage::helper('payu')->__('Thank you.')." ". Mage::helper('payu')->__('The transaction completed successfully.') )
		->save();
		$transaction->save();
	}
	...
	public function updatePaymentStatusError($payment)
	{
		...
		->setParentTransactionId($this->_transactionId)
		->registerVoidNotification();
		$comment = $this->_order->setState(Mage_Sales_Model_Order::STATE_HOLDED, true, "PayU - " . Mage::helper('payu')->__('The transaction is incorrect.') )
		->sendOrderUpdateEmail(true, "PayU - " . Mage::helper('payu')->__('The transaction is incorrect.') )
		->save();
	}
	 ...
	 *
	 * @return PayU_PayU_Model_Config
	 */
	protected function getConfig()
	{
		return Mage::getModel('payu/config');
	}

...
*   CHANGE_LOG:
*   	2012-03-08: 
*			- added localize from config
*			- new goods configuration
*       2012-02-28:
*           - new methods to utilize the goods resources
*/

class PayU_PayU_Model_Config
{
	/** @var string self version */
	protected $_pluginVersion = '0.1.5';
    ...
    protected function setEnvironment(){
    	...
    	switch(Mage::getStoreConfig('payment/payu/environment')){
		
			//check if we work in production mode
			case PayU_PayU_Model_Environment::PRODUCTION:
				$this->_env = "secure";
				$this->_prefix = "";
				break;
	...
	public function getPosAuthKey(){
    	return $this->getStoreConfig('payment/payu/'.$this->_prefix.'pos_auth_key');
    }
    
    /** @return string get (OAuth Client Name) */
	public function getClientId(){
		return $this->getStoreConfig('payment/payu/'.$this->_prefix.'oauth_client_name');
    }
    
    /** @return string get (OAuth Client Secret) */
	public function getClientSecret(){
		return $this->getStoreConfig('payment/payu/'.$this->_prefix.'oauth_client_secret');
    }
    
    /** @return string get signature key */
	public function getSignatureKey(){
		return $this->getStoreConfig('payment/payu/'.$this->_prefix.'signature_key');
    }
    
    /** @return int order validity time in minutes */
	public function getOrderValidityTime(){
		
		$validityTime = $this->getStoreConfig('payment/payu/validity_time');
	...
	public function getNewOrderStatus(){
		return $this->getStoreConfig('payment/payu/order_status');
    }
    
    public function isLatestVersionInstalled(){
    	return !Mage::helper('payu')->isFirstVersionNewer($this->_latestVersion['version'],$this->_pluginVersion);
    }
    
	/** @return int what is the default new order status */
	public function getIsSelfReturnEnabled(){
		return $this->getStoreConfig('payment/payu/selfreturn');
    }
    ...
    protected function localize($string, $s = "/"){
    	return Mage::helper('payu')->localize($string, $s);
    }
